...
def evaluate_model_comprehensive(model, eval_loader, device, stats_tracker=None, dataset_name="eval", return_attention=True, 
                                gaze_weight=0.0, gaze_loss_type='mse', class_weights=None, gaze_loss_scale=1.0):
    ...
    is_input_integration = 'input' in model_name and 'scnet' not in model_name and 'combined' not in model_name
    is_conformer = 'conformer' in model_name
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(eval_loader):
            ...
            # Forward pass with attention - handle different model types
            if is_combined:
                outputs = model(eeg, gaze, return_attention=return_attention)
            elif is_scnet_input or is_input_integration:
                outputs = model(eeg, gaze)
                # Conformer returns (features, logits) tuple
                if is_conformer and isinstance(outputs, tuple):
            ...
            if not (is_scnet_input or is_input_integration):
            ...
            if has_gaze and attention_maps is not None and gaze_weight > 0:
                ...
                batch_loss = (1 - gaze_weight) * cls_loss + gaze_weight * gaze_loss_scaled
