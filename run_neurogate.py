"""
Main entry point for training baseline NeuroGATE model (EEG-only, no gaze integration)
...
from training.trainer_baseline import train_epoch_baseline
...
def train_baseline_neurogate(output_suffix=None, hyp_overrides=None):
    """
    Train baseline NeuroGATE model (EEG-only)
    
    Args:
        output_suffix: Optional suffix for output directory
        hyp_overrides: Optional dictionary to override hyperparameters
    """
    DataDebugger.print_header("BASELINE NEUROGATE (EEG-ONLY)", width=80)
    ...
    output_dir = "neurogate_baseline_results"
    if output_suffix:
    ...
    # Initialize baseline NeuroGATE model
    try:
    ...
    print(f"\nBaseline NeuroGATE Configuration:")
    ...
    es_path = "best_model_baseline.pth"
    ...
    for epoch in range(hyps.epochs):
        DataDebugger.print_header(f"EPOCH {epoch+1}/{hyps.epochs} [BASELINE]", 
                                  width=60, char='-')
        
        # Train
        train_stats = train_epoch_baseline(
        ...
        # Print epoch summary
        print(f"\nEpoch {epoch+1} Summary [BASELINE]:")
    ...
    # Final evaluation
    print("\n" + "=" * 80)
    print(f"FINAL EVALUATION [BASELINE]")
    print("=" * 80)
    ...
    # Save all results
    print("\n" + "=" * 80)
    print(f"SAVING RESULTS [BASELINE]")
    print("=" * 80)
    ...
    with open(stats_tracker.run_dir / 'training_summary.txt', 'w') as f:
        f.write(f"TRAINING SUMMARY REPORT - BASELINE NEUROGATE (EEG-ONLY)\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"Training completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Model type: Baseline NeuroGATE (no gaze integration)\n")
...
def main(output_suffix=None, hyp_overrides=None):
    """
    Main function - trains baseline NeuroGATE model
    
    Args:
        output_suffix: Optional suffix for output directory
        hyp_overrides: Optional dictionary to override hyperparameters
    """
    return train_baseline_neurogate(output_suffix, hyp_overrides)


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Train baseline NeuroGATE model (EEG-only)')
