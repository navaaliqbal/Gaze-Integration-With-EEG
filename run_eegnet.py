"""
Main script to run all EEGNet variants with different gaze integration levels
"""
import os
import sys
import torch
...
from models.model_factory import create_eegnet_model, get_model_config
...
def train_eegnet(integration_type='none', output_suffix=None):
    """
    Train EEGNet with specified gaze integration
    """
    # Setup
    DataDebugger.print_header(f"EEGNET GAZE INTEGRATION: {integration_type.upper()}", width=80)
    ...
    output_dir = f"eegnet_results_{integration_type}_{timestamp}"
    if output_suffix:
    ...
    n_chans = sample_batch['eeg'].shape[1]
    n_times = sample_batch['eeg'].shape[2]
    print(f"\nUsing {n_chans} channels, {n_times} timepoints from data")
    
    # Create model with appropriate parameters
    print(f"\nCreating EEGNet with {integration_type} gaze integration...")
    model = create_eegnet_model(
        integration_type=integration_type,
        num_input=1,
        num_class=hyps.n_outputs,
        channel=n_chans,
        signal_length=n_times,
        fs=hyps.eeg_sampling_rate,
        F1=8,
        D=3,
        dropout_rate=0.2
    ).to(device)
    ...
    # Early stopping
    es_path = f"best_eegnet_{integration_type}.pth"
    ...
    for epoch in range(hyps.epochs):
        print(f"\nEpoch {epoch+1}/{hyps.epochs} [EEGNet-{integration_type.upper()}]")
        print("-" * 60)
    ...
    print(f"\nFinal Results [EEGNet-{integration_type.upper()}]:")
...
def main():
    parser = argparse.ArgumentParser(description='Train EEGNet with gaze integration')
    ...
    print("\n" + "=" * 80)
    print(f"EEGNET TRAINING WITH GAZE INTEGRATION: {args.type.upper()}")
    print("=" * 80)
    
    return train_eegnet(args.type, args.suffix)
